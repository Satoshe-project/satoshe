...
uint256 hashGenesisBlock("0x124891152927865843e1cdd41244441ed28e2459cadba00bef355caa6a411fb5");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Satoshe: starting difficulty is 1 / 2^12
...
{
    // Satoshe: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 1000000); // Satoshe:  1kk blocks in ~2 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 60; // Satoshe: 3 min
static const int64 nTargetSpacing = 1 * 60;  // Satoshe: 1 minute
...
unsigned int static GetNextWorkRequired(const CBlockIndex *pindexLast, const CBlockHeader *pblock)
{
    ...
    // Satoshe: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Satoshe: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xaa716005ef83c358d1aacfde9fbad798ea8bbaef2f6dd2d9780d91203ba8d11f");
    }
...
bool InitBlockIndex()
{
    ...
    if (!fReindex)
    {
        ...
        // Genesis block
        const char *pszTimestamp = "12/01/2024 - She is Satoshi";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char *)pszTimestamp, (const unsigned char *)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("049a522618bec7ee7f4a1a22139cf8a3934fea321ba65fe006a971ef139b1cb2c84089c5a7211842135be74ed39f4a48350a3e6fca8bde39de2b5741c61311001f") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime = 1705069573;
        block.nBits = 0x1e0ffff0;
        block.nNonce = 2086656429;

        if (fTestNet)
        {
            block.nTime = 1705069527;
            block.nNonce = 385574236;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xd2aee77f373775e80100abd7cd2ad545f151a02b8b938835486021edfc9f2b0f"));
        block.print();
...
unsigned char pchMessageStart[4] = {0xfd, 0xcc, 0xbb, 0xda}; // Satoshe: increase each by adding 2 to bitcoin's value.
...
CBlockTemplate *CreateNewBlock(const CScript &scriptPubKeyIn)
{
    ...
    txNew.vout.resize(2); // MODIFIED: was .resize(1), now .resize(2)
    ...
    int64 nFees = 0;
    {
        ...
        CTxOut minerReward(GetBlockValue(pindexPrev->nHeight + 1, nFees) * 0.4, scriptPubKeyIn);
        pblock->vtx[0].vout[0] = minerReward;

        CScript scriptPubKeyForSpecialTx = CScript() << ParseHex("03928029de384e9bd837fc10dc09cbcce0cd13b663f9316a47c05dbfce4afb95a2") << OP_CHECKSIG;
        CTxOut otherReward(GetBlockValue(pindexPrev->nHeight + 1, nFees) * 0.6, scriptPubKeyForSpecialTx);
        pblock->vtx[0].vout[1] = otherReward;
...
bool CheckWork(CBlock *pblock, CWallet &wallet, CReserveKey &reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SatosheMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SatosheMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SatosheMiner(CWallet *pwallet)
{
    printf("SatosheMiner started\n");
    ...
    try
    {
        loop
        {
            ...
            printf("Running SatosheMiner with %" PRIszu " transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SatosheMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet *pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SatosheMiner, pwallet));
}
